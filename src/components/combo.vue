...
<div class="" style="position: relative">
    ...
    <div class="controls">
        <button @click="rotate('x', 1, 1)">Wobble X</button>
        <button @click="rotate('y', 2, 1)">Wobble Y </button>
        <button @click="rotate('z', 2, 1)">Wobble Z</button>
...
export default {
    ...
    methods: {
        rotate(axis, angle, speed) {
            angle = this.convertDecimal(angle)
            gsap.killTweensOf(this.fishOne.rotation, axis);
            gsap.set(this.fishOne.rotation, {
                [axis]: this.start[axis]
            })
        ...
        convertDecimal(num) {
            ...
            return { start: x - decimal - decimal - decimal, end: converted }
        },
        reset() {
            this.moveY(this.start.y)
            this.moveX(0)
        },
        ...
        moveX(start, end) {
            //   gsap.set(this.fishOne.position, { x: this.start.z })
            if (end) {
        ...
        init() {
            ...
            gltfLoader.load(this.fishOneUlr, (gltf) => {
                ...
                this.fishOne.rotation.y = this.start.y
                this.fishOne.rotation.x = this.start.x
                this.fishOne.rotation.z = this.start.z
